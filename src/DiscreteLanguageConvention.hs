...
module DiscreteLanguageConvention where


import Control.Monad.Bayes.Class (MonadDistribution (normal, random, uniformD), normalPdf, MonadFactor (score), MonadMeasure, condition)
import Data.Vector ()
...
data UpDown = Up | Down deriving (Show, Eq, Ord, Read)
data LeftRight = Left' | Right' deriving (Show, Eq, Ord, Read)
...
newtype AgentAction (i :: AgentNumber) = AgentAction {_agentAction :: Maybe (UpDown, LeftRight)} deriving Show
...
newtype Language where
    Language :: {_coords :: [(UpDown, LeftRight)]} -> Language
    deriving Show
$(makeLenses ''Language)
...
interpret :: Language -> V2 Double -> (UpDown, LeftRight)
interpret (Language lang) pos = applyLanguage lang result where
    result
        | pos ^. _x > 0 && pos ^. _y > 0 = (Up, Right')
        | pos ^. _x > 0 && pos ^. _y < 0 = (Down, Right')
        | pos ^. _x < 0 && pos ^. _y > 0 = (Up, Left')
        | pos ^. _x < 0 && pos ^. _y < 0 = (Down, Left')
...
agentIPrior (Depth 0) agentID = proc (action, staticPrior) -> do
    ...
    lang <- performOnFirstSample $ uniformD [constM $ pure x | x <- languages] -< ()
    -- let lang = [(Up, Right'), (Down, Right'), (Up, Left'), (Down, Left')]
    -- let lang = undefined 

    -- case (agentID, staticPrior) of
    ...
    let otherAction' = interpret (Language lang) (V2 ballX ballY)
    otherAction <- arrM (\x -> uniformD [Just x, Nothing]) -< otherAction'
...
agentIPrior (Depth d) agentID =
    feedback (AgentAction (Just $ (Up, Left')) :: AgentAction (Other i))
    proc ((action, userData), otherAgentAction) -> do
        ...
        lang <- constM $ uniformD languages -< () -- brownianMotion1D &&& brownianMotion1D -< 8
        obs <- observationModel -< state
        (newOtherAgentAction, _) <- agentI (Depth (d-1)) (other agentID) -< (obs, userData)
        returnA -< ((state, Language lang), newOtherAgentAction)
...
agentIPosterior depth agentID = proc (observation, agentIAction, userData) -> do
    ...
    -- arrM condition -<  utt == predictedUtt
    case (utt, predictedUtt) of
            (Just u, Just u') -> arrM condition -<  utt == predictedUtt
            _ -> returnA -< ()
...
--     -- return -< undefined
--     action <- undefined -< () -- AgentAction . Just <$> Example.prior -< ()
...
produce :: V2 Double -> (UpDown, LeftRight)
produce (V2 x y)
    | x > 0 && y > 0 = (Up, Right')
    | x > 0 && y < 0 = (Down, Right')
    | x < 0 && y > 0 = (Up, Left')
...
languages :: [[(UpDown, LeftRight)]]
-- languages = replicateM 4 [(Up, Right'), (Down, Right'), (Up, Left'), (Down, Left')]
languages = [[(Up, Right'), (Down, Right'), (Up, Left'), (Down, Left')], reverse [(Up, Right'), (Down, Right'), (Up, Left'), (Down, Left')]]

applyLanguage :: [(UpDown, LeftRight)] -> (UpDown, LeftRight) -> (UpDown, LeftRight)
applyLanguage lang inp = lang !! i where
    i = case inp of
        (Up, Right') -> 0
        (Down, Right') -> 1
        (Up, Left') -> 2
...
agentI depth i = feedback (AgentAction $ Just (Up, Left')) proc ((observation, userData), act) -> do
    ...
    actionDist <- case depth of
        ...
        Depth 0 -> returnA -< first (\(s, Language l) -> applyLanguage l $ produce $ (^. ball) s) <$> beliefDist
        _ -> undefined -< () -- particleFilter params{n=numParticles} (decisionModelAgentI depth i) -< (stateBelief, userData)
    nextAct <- arrM empirical -< actionDist
    let encoded = AgentAction $ Just nextAct -- nextAct & agentAction . _Just %~ (+ langBelief ^. coords)
...
main = feedback (AgentAction Nothing, AgentAction Nothing) proc (userInput, actions) -> do
    ...
    (buttonPic, buttonOn) <- button buttonParams{
        buttonPos=V2 200 300, buttonColor=red, buttonText = "communicate", buttonInitialVal = True} -< userInput
    ...
    let dict1 = foldr (\(k, v) y -> Map.insertWith (+) k v y) (Map.empty) $ first ((\(Language x) -> if x==[(Up, Right'), (Down, Right'), (Up, Left'), (Down, Left')] then True else False) . snd) <$> beliefAgent1
    let dict2 = foldr (\(k, v) y -> Map.insertWith (+) k v y) (Map.empty) $ first ((\(Language x) -> if x==[(Up, Right'), (Down, Right'), (Up, Left'), (Down, Left')] then True else False) . snd) <$> beliefAgent2
    ...
    let pic4 = displayAction green $ newActions ^. _1 . agentAction . to (fromMaybe (Up, Left'))
    let pic5 = displayAction yellow $ newActions ^. _2 . agentAction . to (fromMaybe (Up, Left'))
    ...
    returnA -< (
            ...
            <> languagePic
            <> scale 0.1 0.1 (text (show dict1))
            <> translate 0 (-50) (scale 0.1 0.1 (text (show dict2)))
            <> buttonPic
            <> a1obsXPic
            <> a1obsYPic
            <> a2obsXPic
...
displayAction col (ud, lr) = translate (case lr of {Left' -> -100; Right' -> 100}) (case ud of {Up -> 100; Down -> -100}) (color col $ rectangleSolid 100.0 100.0)
