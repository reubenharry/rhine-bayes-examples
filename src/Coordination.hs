...
import Control.Monad.Bayes.Class (MonadSample (normal, random), normalPdf)
import Data.Vector ()
...
import MutualStoch ( expected, normalPdf2D, renderObjects )
...
import MutualStoch (drawParticle')
...
main = feedback (AgentAction Nothing, AgentAction Nothing) proc (glossInput, actions) -> do
...
painfulExpectation state =
    ...
    in State {_ball = expected f1, _actions = (AgentAction $ undefined, AgentAction $ undefined)}
